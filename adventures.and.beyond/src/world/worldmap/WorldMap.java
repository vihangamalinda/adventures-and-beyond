...
public class WorldMap extends AbstractWorldMap {
    public WorldMap(int[][] mapTileMatrix,
                    String worldMapKey) {
        super(mapTileMatrix,
              worldMapKey);
    }
    ...
    @Override
    public int getTileKey(int rowIndex,
                          int colIndex) {
        return this.getTileKeyByRowAndCol(rowIndex,
                                          colIndex);
    }

    public boolean canTileBeCollided(int rowIndex,
                                     int colIndex) {
        int tileKey = this.getTileKeyByRowAndCol(rowIndex,
                                                 colIndex);
    ...
    @Override
    protected void drawWorldMap(Graphics2D graphics2D,
                                int drawMapRow,
                                int drawMapCol) {
        for (int windowRow = 0; windowRow < Constant.MAX_SCREEN_ROW; windowRow++) {
            ...
            for (int windowCol = 0; windowCol < MAX_SCREEN_COLUMN; windowCol++) {
                ...
                drawScale(graphics2D,
                          windowPositionX,
                          windowPositionY);
                drawRowAndColNumbers(graphics2D,
                                     drawMapRow,
                                     currentMapColumn,
                                     windowPositionX,
                                     windowPositionY);
    ...
    private void drawRowAndColNumbers(Graphics2D graphics2D,
                                      int row,
                                      int col,
                                      int windowStartX,
                                      int windowStartY) {
    ...
    protected void drawScale(Graphics2D graphics2D,
        ...
        graphics2D.drawRect(windowPositionX,
                            windowPositionY,
                            TILE_SIZE,
                            TILE_SIZE);
